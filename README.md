...
**五、失败面板自己也要被验证过 —— 加一个故障注入开关。**

这一条是最后补上的，也是这一节里最值得记的：

`drawBootError`（小游戏侧）写好了、打包了、看着没问题，
但**从来没有被渲染过一次** —— 因为它只在启动失败时才跑，
而正常路径永远走不到它。加 `?mgfail=1` 做注入验证，第一次跑就发现它是坏的：

```
canvas: 300x150   ← 宿主的默认尺寸，不是屏幕尺寸
截图：文字被放大十几倍，一屏只剩「3D 画面起不来」几个字，其余全被裁掉
```

根因：`Core.init` 里**先创建渲染器、再 resize**。渲染器一失败，
画布就停在默认的 300x150 上，而它会被拉伸到全屏显示 ——
**「启动失败该给玩家看什么」这个问题，本身又变成了一次失败。**

三处修补：

1. `drawBootError` **自己**把画布定到屏幕尺寸（算法与 `Core.resize` 一致：
   逻辑尺寸 × `min(dpr, 2)`）。不能指望它已经是对的。
2. **截断必须留痕。** 裸 `slice` 出来的值看起来就像「值本来就是这样」——
   渲染器那一行显示成 `ANGLE (Google, Vulkan 1.3.0 (Swift`，括号都不闭合，
   看的人分不清是被截了还是真长这样。现在加省略号：
   **一个会误导人的诊断字段，比没有这个字段更糟。**
3. 诊断里加 `bootErrDrawn`（`null` = 没走到失败路径）。
   不加它的话，「画了一张能看的错误页」和「画了一张被拉伸到只剩几个字的图」
   在诊断里**完全一样**（都是 `state:'ready'`），只能靠人去看截图 ——
   而这段代码本来就是因为「没人看过」才坏掉的。

故障注入开关的位置是**刻意的**：`?mgfail=1` 必须抛在
`takeScreenCanvas()` **之后**、`Core.init()` **之前**。
提前抛的话 `drawBootError` 连画布都拿不到，会静默什么都不画 ——
于是「注入的故障」和「真的故障」表现得不一样，这个开关就白加了。
真实失败正是发生在 `Core.init` 里，顺序必须一致。

两个方向都验过：

```
?mgfail=1  → bootErrDrawn: true, canvas 500x860, 截图完整可读
?mgplay=8  → 无 bootErrDrawn 行, state: playing, t: 8.5
```

> 一般化：**「代码写好了」和「代码被执行过」是两件事。**
> 只在失败时才跑的那段代码，天然躲过所有正常路径的验证 ——
> 而它又恰恰是玩家在最需要帮助的那一刻看到的唯一东西。
> 给它加一个故障注入开关，把「只在出错时发生」变成一条可复现的输入。
...
| `?mgfail=1` | **故障注入**：在拿到上屏画布之后、`Core.init` 之前抛一个 WebGL 错误，把「启动失败」这条路真的走一遍 |

> `?mgfail=1` 为什么必须有：`drawBootError` 那段代码**从来没有被渲染过** ——
> 写好了、打包了、看着没问题，但「字会不会排到屏幕外」「中文换行对不对」
> 「探测表会不会压住技术细节」一次都没人看过。而它正是玩家在真机上
> 唯一会看到的东西（Web 侧那次真实失败就是这么来的）。
>
> 抛出点的位置是刻意的：必须在 `takeScreenCanvas()` **之后**、`Core.init()`
> **之前**。因为 `drawBootError` 要往 `screenCanvas` 上画，提前抛的话
> 它连画布都拿不到，会静默什么都不画 —— 于是「注入的故障」和「真的故障」
> 表现得不一样，这个开关就白加了。真实失败正是发生在 `Core.init` 里，顺序一致。
>
